...
const adminKeysStore: any[] = [];

export async function GET() {
  return NextResponse.json(adminKeysStore);
}

export async function POST(req: Request) {
  try {
    const body = await req.json().catch(() => ({}));
    const name = body.name || "Admin Key";
    const planType = body.planType || "diamond";
    ...
    adminKeysStore.unshift(apiKeyItem);
